...
pygame.display.set_caption('VMs Adventure Quest')
...
MAX_LEVELS = 3
MISSIONS = {1: 'Welcome To Terraria', 2: "      Into The Woods", 3: "    Welcome to the last level "}
...
back_img=pygame.image.load(f'back_btn.png').convert_alpha()
...
sky_img = pygame.image.load(f'assets/img/background/level{level}/sky1.png').convert_alpha()
...
# Add this new function to create the database and table
def create_database():
...
def update_score_in_db(username, score):
    conn = sqlite3.connect('game_data.db')
    c = conn.cursor()
    c.execute("INSERT OR REPLACE INTO players (username, score) VALUES (?, ?)", (username, score))
    conn.commit()
    conn.close()
def save_username(username):
    conn = sqlite3.connect('game_data.db')
    c = conn.cursor()
    c.execute("INSERT OR REPLACE INTO players (username) VALUES (?)", (username,))
    conn.commit()
    conn.close()
def draw_high_scores(screen, high_scores):
    ...
    for i, header in enumerate(headers):
        text = font.render(header, True, RED)
        screen.blit(text, (SCREEN_WIDTH // 4 + i * (SCREEN_WIDTH // 4) - text.get_width() // 2, header_y))

    # Draw table content
    y = 160
    for rank, username, score in high_scores:
        rank_text = font.render(str(rank), True, WHITE)
        name_text = font.render(username, True, WHITE)
        score_text = font.render(str(score), True, WHITE)
        
        screen.blit(rank_text, (SCREEN_WIDTH // 4 - rank_text.get_width() // 2, y))
        screen.blit(name_text, (SCREEN_WIDTH // 2 - name_text.get_width() // 2, y))
        screen.blit(score_text, (3 * SCREEN_WIDTH // 4 - score_text.get_width() // 2, y))
        y += 40

    back_btn = Button(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT - 100, back_img, 1)
    return back_btn.draw(screen)
...
class Soldier(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.soldier_type == 'binder':  # Only update score for the player
            self.update_score()

    def update_score(self):
        for enemy in enemy_group:
            if not enemy.alive and not enemy.score_added:
                self.score += 15  # Increase score by 10 for each killed enemy
        ...
        if self.rect.colliderect(player):
            if self.alive:
                ...
                damage_text = DamageText(self.rect.centerx,
                                         self.rect.centery, str(" speed increased "), WHITE)
            ...
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    if tile >= 0 and tile <= 8:
                        ...
                        player = Soldier('binder', x * TILE_SIZE, y * TILE_SIZE, self.player_scale, 5, 20, 5, 100)
    ...
    def update(self):
        ...
        if pygame.sprite.collide_rect(self, player):
            ...
            if self.item_type == 'Health':
                player.health += 35
                damage_text = DamageText(self.rect.centerx,
                                         self.rect.centery, f"Health +45", WHITE)
                damage_text_group.add(damage_text)
                ...
                player.ammo += 25
                damage_text = DamageText(self.rect.centerx,
                                         self.rect.centery, f"Ammo +25", WHITE)
                damage_text_group.add(damage_text)
            elif self.item_type == 'Grenade':
                player.grenades += 5
                damage_text = DamageText(self.rect.centerx,
                                         self.rect.centery, f"Grenade +5", WHITE)
                damage_text_group.add(damage_text)
            # remove the box
            self.kill()
                ...
                if pygame.sprite.spritecollide(live_entity, bullet_group, False):
                    if live_entity.alive:
                        ...
                        damage_text = DamageText(live_entity.rect.centerx,
                                                 live_entity.rect.centery, str("You have been cursed"),RED )
                        damage_text_group.add(damage_text)
                        if player.alive:RED
                        player.health -= 50
                        player.ammo -= 5
                ...
                if live_entity.entity_type != "chest":
                    if abs(self.rect.centerx - live_entity.rect.centerx) < TILE_SIZE * 2 and \
                        ...
                        if live_entity.alive:
                            live_entity.health = -25
                            damage_text = DamageText(live_entity.rect.centerx,
                                                     live_entity.rect.centery, str("you will be cursed"), RED)
                            damage_text_group.add(damage_text)
                            if player.alive:
                                player.health -= 50
                            self.kill()
...
death_fade = ScreenFade(2, BLACK, 10)

# create buttons
start_btn = Button(SCREEN_WIDTH - 515, SCREEN_HEIGHT - 510, start_img, 1)
exit_btn = Button(SCREEN_WIDTH // 2 - 110, SCREEN_HEIGHT // 2 - 10, exit_img, 1)
...
world = World()
world.set_player_scale(3)  # Set the player scale to 2.5, for example
...
# Load a new image for the high scores button
high_scores_img = pygame.image.load('assets/img/buttons/user1.png').convert_alpha()
high_scores_btn = Button(SCREEN_WIDTH - 280, SCREEN_HEIGHT - 100, high_scores_img, 0.5)
...
while running:
    ...
    if show_high_scores:
        ...
        if draw_high_scores(screen, high_scores):
            show_high_scores = False
            username_screen = True

    elif not start_game and not username_screen:
        # Main Menu
    ...
    elif username_screen:
        # Username input screen
        screen.fill(BLACK)
        
        input_box = pygame.Rect(SCREEN_WIDTH // 4, SCREEN_HEIGHT // 2 - 32, SCREEN_WIDTH // 2, 64)
        pygame.draw.rect(screen, RED, input_box, 2)

        prompt_text = font.render("Enter your username:", True, WHITE)
        screen.blit(prompt_text, (SCREEN_WIDTH // 4, SCREEN_HEIGHT // 2 - 64))

        input_surface = font.render(input_text, True, WHITE)
        screen.blit(input_surface, (input_box.x + 5, input_box.y + 5))

        if input_active and (pygame.time.get_ticks() // 500) % 2 == 0:
            cursor_pos = font.size(input_text)[0]
            pygame.draw.line(screen, WHITE, (input_box.x + 5 + cursor_pos, input_box.y + 5),
                             (input_box.x + 5 + cursor_pos, input_box.y + input_box.height - 10), 2)

        start_game_btn = Button(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 50, start_img, 1)
        if start_game_btn.draw(screen):
            if input_text.strip():
        ...
        if high_scores_btn.draw(screen):
        ...
        draw_text(f'Grenade: {player.grenades}', font, RED, 10, 60)
        draw_text(f'Player: {username}', font, RED, 10, 85)
        draw_text(f'Score: {player.score}', font, RED, 10, 110)
        draw_lvl_info_text(f'Mission {level}', font, RED, SCREEN_WIDTH // 2 , 0)
        draw_lvl_info_text(f'{MISSIONS[level]}', font, RED, SCREEN_WIDTH // 2 -50, 20)
        ...
        if player.alive:
            ...
            if level_complete:
                update_score_in_db(username, player.score)
                start_intro = True
                level += 1
                bg_scroll = 0
                    ...
                    with open(f'level{level}_data.csv', newline='') as csvfile:
                        ...
                        for x, row in enumerate(reader):
                            for y, tile in enumerate(row):
                                world_data[x][y] = int(tile)
                    world = World()
                    world.set_player_scale(3)  # Set the player scale to 3, for example
                    player, health_bar = world.process_data(world_data)
    ...
    for event in pygame.event.get():
        ...
        if username_screen or show_high_scores:
